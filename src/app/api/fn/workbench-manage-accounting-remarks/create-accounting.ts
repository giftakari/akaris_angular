...
export interface CreateAccounting$Params {
...
export function createAccounting(http: HttpClient, rootUrl: string, params: CreateAccounting$Params, context?: HttpContext): Observable<StrictHttpResponse<AccountingResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, createAccounting.PATH, 'post');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<AccountingResponseWrapper>;
    })
  );
}

createAccounting.PATH = '/book/accounting/reservationworkbench/{ReservationResource_Identifier}/accountings';
