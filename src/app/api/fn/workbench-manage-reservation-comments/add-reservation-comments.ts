...
import { ReservationCommentListRequest } from '../../models/reservation-comment-list-request';
import { ReservationCommentListResponseWrapper } from '../../models/reservation-comment-list-response-wrapper';

export interface AddReservationComments$Params {
 ...
 */
  XAUTH_TRAVELPORT_ACCESSGROUP?: string;
      body: ReservationCommentListRequest
}

export function addReservationComments(http: HttpClient, rootUrl: string, params: AddReservationComments$Params, context?: HttpContext): Observable<StrictHttpResponse<ReservationCommentListResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, addReservationComments.PATH, 'post');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<ReservationCommentListResponseWrapper>;
    })
  );
}

addReservationComments.PATH = '/book/remarks/reservationworkbench/{ReservationResource_Identifier}/reservationcomments/list';
