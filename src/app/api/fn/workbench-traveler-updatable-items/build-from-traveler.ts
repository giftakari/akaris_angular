...
import { TravelerUpdatableItemsQueryBuildFromTraveler } from '../../models/traveler-updatable-items-query-build-from-traveler';

export interface BuildFromTraveler$Params {
 ...
 */
  TravelportPlusSessionID?: string;
      body?: TravelerUpdatableItemsQueryBuildFromTraveler
}

export function buildFromTraveler(http: HttpClient, rootUrl: string, params: BuildFromTraveler$Params, context?: HttpContext): Observable<StrictHttpResponse<TravelerUpdatableItemsListResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, buildFromTraveler.PATH, 'post');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<TravelerUpdatableItemsListResponseWrapper>;
    })
  );
}

buildFromTraveler.PATH = '/book/updateableitem/reservationworkbench/{ReservationResource_Identifier}/travelerupdatableitems/buildfromtraveler';
