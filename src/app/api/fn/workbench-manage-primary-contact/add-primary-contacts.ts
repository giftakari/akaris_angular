...
import { PrimaryContactListRequest } from '../../models/primary-contact-list-request';
import { PrimaryContactListResponseWrapper } from '../../models/primary-contact-list-response-wrapper';

export interface AddPrimaryContacts$Params {
 ...
 */
  XAUTH_TRAVELPORT_ACCESSGROUP?: string;
      body: PrimaryContactListRequest
}

export function addPrimaryContacts(http: HttpClient, rootUrl: string, params: AddPrimaryContacts$Params, context?: HttpContext): Observable<StrictHttpResponse<PrimaryContactListResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, addPrimaryContacts.PATH, 'post');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<PrimaryContactListResponseWrapper>;
    })
  );
}

addPrimaryContacts.PATH = '/book/primarycontact/reservationworkbench/{ReservationResource_Identifier}/primarycontacts/list';
