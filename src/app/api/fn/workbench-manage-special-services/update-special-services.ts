...
export interface UpdateSpecialServices$Params {
...
export function updateSpecialServices(http: HttpClient, rootUrl: string, params: UpdateSpecialServices$Params, context?: HttpContext): Observable<StrictHttpResponse<SpecialServiceResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, updateSpecialServices.PATH, 'put');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<SpecialServiceResponseWrapper>;
    })
  );
}

updateSpecialServices.PATH = '/book/specialservices/reservationworkbench/{ReservationResource_Identifier}/specialservices\'';
