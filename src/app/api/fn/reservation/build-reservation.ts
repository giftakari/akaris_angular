...
import { ReservationQueryBuildWrapper } from '../../models/reservation-query-build-wrapper';
import { ReservationResponseWrapper } from '../../models/reservation-response-wrapper';

export interface BuildReservation$Params {
 ...
 */
  TravelportPlusSessionID?: string;
      body: ReservationQueryBuildWrapper
}

export function buildReservation(http: HttpClient, rootUrl: string, params: BuildReservation$Params, context?: HttpContext): Observable<StrictHttpResponse<ReservationResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, buildReservation.PATH, 'post');
  if (params) {
...
buildReservation.PATH = '/book/reservation/reservations/build';
