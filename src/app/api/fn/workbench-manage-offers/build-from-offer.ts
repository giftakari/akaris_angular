...
import { OfferQueryBuildFromOffer } from '../../models/offer-query-build-from-offer';
import { OfferResponseWrapper } from '../../models/offer-response-wrapper';

export interface BuildFromOffer$Params {
 ...
 */
  TravelportPlusSessionID?: string;
      body?: OfferQueryBuildFromOffer
}

export function buildFromOffer(http: HttpClient, rootUrl: string, params: BuildFromOffer$Params, context?: HttpContext): Observable<StrictHttpResponse<OfferResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, buildFromOffer.PATH, 'post');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<OfferResponseWrapper>;
    })
  );
}

buildFromOffer.PATH = '/book/airoffer/reservationworkbench/{ReservationResource_Identifier}/offers/buildfromoffer';
