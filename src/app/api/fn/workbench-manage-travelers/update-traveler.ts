...
export interface UpdateTraveler$Params {
...
export function updateTraveler(http: HttpClient, rootUrl: string, params: UpdateTraveler$Params, context?: HttpContext): Observable<StrictHttpResponse<TravelerResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, updateTraveler.PATH, 'put');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<TravelerResponseWrapper>;
    })
  );
}

updateTraveler.PATH = '/book/traveler/reservationworkbench/{ReservationResource_Identifier}/travelers/{id}';
