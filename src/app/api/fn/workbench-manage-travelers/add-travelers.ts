...
import { TravelerListRequest } from '../../models/traveler-list-request';
import { TravelerListResponseWrapper } from '../../models/traveler-list-response-wrapper';

export interface AddTravelers$Params {
 ...
 */
  TravelportPlusSessionID?: string;
      body: TravelerListRequest
}

export function addTravelers(http: HttpClient, rootUrl: string, params: AddTravelers$Params, context?: HttpContext): Observable<StrictHttpResponse<TravelerListResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, addTravelers.PATH, 'post');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<TravelerListResponseWrapper>;
    })
  );
}

addTravelers.PATH = '/book/traveler/reservationworkbench/{ReservationResource_Identifier}/travelers/list';
