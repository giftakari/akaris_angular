...
import { TravelerQueryUpdate } from '../../models/traveler-query-update';

export interface UpdateFromTravelerUpdatedItems$Params {
 ...
 */
  TravelportPlusSessionID?: string;
      body: TravelerQueryUpdate
}

export function updateFromTravelerUpdatedItems(http: HttpClient, rootUrl: string, params: UpdateFromTravelerUpdatedItems$Params, context?: HttpContext): Observable<StrictHttpResponse<void>> {
  const rb = new RequestBuilder(rootUrl, updateFromTravelerUpdatedItems.PATH, 'put');
  if (params) {
...
updateFromTravelerUpdatedItems.PATH = '/book/traveler/reservationworkbench/{ReservationResource_Identifier}/travelers/updatefromtravelerupdateditems/{id}';
