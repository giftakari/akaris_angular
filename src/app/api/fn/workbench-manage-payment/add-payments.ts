...
import { PaymentListRequest } from '../../models/payment-list-request';
import { PaymentListResponseWrapper } from '../../models/payment-list-response-wrapper';

export interface AddPayments$Params {
...
export function addPayments(http: HttpClient, rootUrl: string, params: AddPayments$Params, context?: HttpContext): Observable<StrictHttpResponse<PaymentListResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, addPayments.PATH, 'post');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<PaymentListResponseWrapper>;
    })
  );
}

addPayments.PATH = '/paymentoffer/reservationworkbench/{ReservationResource_Identifier}/payments/list';
