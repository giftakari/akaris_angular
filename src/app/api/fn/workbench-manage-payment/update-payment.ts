...
export function updatePayment(http: HttpClient, rootUrl: string, params: UpdatePayment$Params, context?: HttpContext): Observable<StrictHttpResponse<PaymentResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, updatePayment.PATH, 'put');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<PaymentResponseWrapper>;
    })
  );
}

updatePayment.PATH = '/paymentoffer/reservationworkbench/{ReservationResource_Identifier}/payments';
