...
import { FormOfPaymentListRequest } from '../../models/form-of-payment-list-request';
import { FormOfPaymentListResponseWrapper } from '../../models/form-of-payment-list-response-wrapper';

export interface AddFormOfPayments$Params {
 ...
 */
  TravelportPlusSessionID?: string;
      body: FormOfPaymentListRequest
}

export function addFormOfPayments(http: HttpClient, rootUrl: string, params: AddFormOfPayments$Params, context?: HttpContext): Observable<StrictHttpResponse<FormOfPaymentListResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, addFormOfPayments.PATH, 'post');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<FormOfPaymentListResponseWrapper>;
    })
  );
}

addFormOfPayments.PATH = '/payment/reservationworkbench/{ReservationResource_Identifier}/formofpayments/list';
