...
import { ReceiptQueryBuildFromPayment } from '../../models/receipt-query-build-from-payment';

export interface BuildReceiptsFromPayment$Params {
 ...
 */
  TravelportPlusSessionID?: string;
      body: ReceiptQueryBuildFromPayment
}

export function buildReceiptsFromPayment(http: HttpClient, rootUrl: string, params: BuildReceiptsFromPayment$Params, context?: HttpContext): Observable<StrictHttpResponse<ReceiptListResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, buildReceiptsFromPayment.PATH, 'post');
  if (params) {
...
buildReceiptsFromPayment.PATH = '/receipt/reservations/{ReservationResource_Identifier}/receipts/buildfrompayment';
