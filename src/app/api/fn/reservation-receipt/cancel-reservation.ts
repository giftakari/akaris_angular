...
export function cancelReservation(http: HttpClient, rootUrl: string, params: CancelReservation$Params, context?: HttpContext): Observable<StrictHttpResponse<ReceiptListResponseWrapper>> {
  const rb = new RequestBuilder(rootUrl, cancelReservation.PATH, 'post');
  if (params) {
    rb.path('ReservationResource_Identifier', params.ReservationResource_Identifier, {"style":"simple","explode":false});
    rb.query('OfferIdentifier', params.OfferIdentifier, {"style":"form","explode":true});
    rb.query('agencySettlementNotReportedInd', params.agencySettlementNotReportedInd, {"style":"form","explode":true});
...
cancelReservation.PATH = '/receipt/reservations/{ReservationResource_Identifier}/receipts';
