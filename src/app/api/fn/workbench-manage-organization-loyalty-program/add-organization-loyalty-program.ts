...
export interface AddOrganizationLoyaltyProgram$Params {
 ...
 */
  TravelportPlusSessionID?: string;
      body?: OrganizationLoyaltyProgram
}

export function addOrganizationLoyaltyProgram(http: HttpClient, rootUrl: string, params: AddOrganizationLoyaltyProgram$Params, context?: HttpContext): Observable<StrictHttpResponse<OrganizationLoyaltyProgramResponse>> {
  const rb = new RequestBuilder(rootUrl, addOrganizationLoyaltyProgram.PATH, 'post');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<OrganizationLoyaltyProgramResponse>;
    })
  );
}

addOrganizationLoyaltyProgram.PATH = '/book/reservationworkbench/{ReservationResource_Identifier}/organizationloyaltyprograms';
